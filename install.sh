...
start_timer() {
    ...
    _oldpidfile=$(ls /tmp/vllm-timer-pid-${_cid}-* 2>/dev/null | head -1)
...
if [ -n "$VLLM_HOST_IP" ] && [ "$VLLM_HOST_IP" != "0.0.0.0" ]; then
    PORT_BIND="-p ${VLLM_HOST_IP}:${PORT}:8000"
else
...
DTYPE="bfloat16"
MODEL_PATH="blackforestlabs/FLUX.2-dev"
SERVED_MODEL_NAME="FLUX.2-dev"
...
echo ""
if [ -n "$VLLM_HOST_IP" ] && [ "$VLLM_HOST_IP" != "0.0.0.0" ]; then
    echo "[INFO] API port ${PORT} bound on ${VLLM_HOST_IP} (nc -zv ${VLLM_HOST_IP} ${PORT} to test)."
